...
const initEditor = async (startServer = true, standaloneServer, content): Promise<void> => {
  ...
  if (startServer) loadTemplate(editor, standaloneServer, content)
}

const loadTemplate = async (editor, standaloneServer, editorContent): Promise<void> => {
  const baseUrl = standaloneServer ? `http://localhost:${port}` : ''
