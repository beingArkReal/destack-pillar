...
const ContentProvider = ({
  ...
  React.useEffect(() => {
    if (mounted.current)
      return;
    if (showEditor) {
      Promise.resolve().then(function () { return require('./initEditor-85571062.js'); }).then((c) => c.initEditor(startServer, standaloneServer, content));
    } else {
      ...
      if (template) {
        const content2 = JSON.parse(template.content);
        setCss(content2.css);
        setHtml(content2.html);
      }
    }
    mounted.current = true;
  }, []);
